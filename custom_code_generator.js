...
function workspaceToCodeDebug(workspace) {
    ...
    if (!Blockly.Java.isInitialized) {
        Blockly.Java.init(workspace);
    }
    ...
    for (let i = 0, block; (block = blocks[i]); i++) {
        ...
        if (line) {
            if (block.outputConnection) {
                ...
                line = Blockly.Java.scrubNakedValue(line);
                if (Blockly.Java.STATEMENT_PREFIX && !block.suppressPrefixSuffix) {
                    line = Blockly.Java.injectId(Blockly.Java.STATEMENT_PREFIX, block) + line;
                }
                if (Blockly.Java.STATEMENT_SUFFIX && !block.suppressPrefixSuffix) {
                    line = line + Blockly.Java.injectId(Blockly.Java.STATEMENT_SUFFIX, block);
                }
            }
            code.push(line);
        }
...
function blockToCodeDebug(block, opt_thisOnly) {
    if (Blockly.Java.isInitialized === false) {
        console.warn(
    ...
    if (!block.isEnabled()) {
        ...
        return opt_thisOnly ? '' : Blockly.Java.blockToCode(block.getNextBlock());
    }
    if (block.isInsertionMarker()) {
        // Skip past insertion markers.
        return opt_thisOnly ? '' : Blockly.Java.blockToCode(block.getChildren(false)[0]);
    }

    const func = Blockly.Java[block.type];
    if (typeof func !== 'function') {
        throw Error(
            'Language "' + Blockly.Java.name_ + '" does not know how to generate ' +
    ...
    if (Array.isArray(code)) {
        ...
        var c = [Blockly.Java.scrub_(block, code[0], opt_thisOnly), code[1]];

        document.getElementById("debugText1").innerHTML += c[0] + "<br>";
        return c;
    } else if (typeof code === 'string') {
        if (Blockly.Java.STATEMENT_PREFIX && !block.suppressPrefixSuffix) {
            code = Blockly.Java.injectId(Blockly.Java.STATEMENT_PREFIX, block) + code;
        }
        if (Blockly.Java.STATEMENT_SUFFIX && !block.suppressPrefixSuffix) {
            code = code + Blockly.Java.injectId(Blockly.Java.STATEMENT_SUFFIX, block);
        }
        var c = Blockly.Java.scrub_(block, code, opt_thisOnly);
